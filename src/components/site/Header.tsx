    ...
    <header className="sticky top-0 z-40 backdrop-blur-xl bg-[oklch(0.985_0.008_90/0.78)] border-b border-gold/30 shadow-[0_2px_24px_-12px_oklch(0.36_0.12_25/0.25)]">
      ...
      <div className="container mx-auto px-4 lg:px-8 flex items-center justify-between gap-6 lg:gap-12 h-20 lg:h-24">
        ...
        <Link to="/" className="flex items-center gap-4 group shrink-0" onClick={() => setOpen(false)}>
          <img src={logo} alt={brand} className="h-12 lg:h-14 w-auto" />
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <button
            ...
            onClick={() => setOpen((v) => !v)}
            className="flex items-center gap-2 px-3 py-2 -mr-1 font-display text-sm tracking-wide text-bordo rounded-md border border-gold/40 bg-cream/60 shadow-[inset_0_1px_0_oklch(1_0_0/0.7),0_2px_6px_-2px_oklch(0.36_0.12_25/0.25)]"
          >
...
function LanguageSwitcher({
  ...
  return (
    <div
      className={`inline-flex items-center rounded-md border border-gold/40 bg-cream/60 shadow-[inset_0_1px_0_oklch(1_0_0/0.7)] overflow-hidden ${compact ? "ml-0" : "ml-2"}`}
      role="group"
      aria-label="Language"
    >
