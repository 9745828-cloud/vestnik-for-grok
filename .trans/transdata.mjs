...
const LANGS = { zh: "Simplified Chinese", ar: "Arabic" };
const TARGET = 130;

function sys(langName) {
  return `You translate a fragment of a TypeScript source file from English into ${langName}.
STRICT RULES:
...
- Do NOT translate or alter: import statements, identifiers, object keys, numbers, booleans, hex colors, CSS values such as "center 34%", URLs, domain names (culture.ru, ru.wikipedia.org, etc.), email addresses, route paths starting with "/", and any "id" or slug field values.
- Transliterate proper personal names naturally for ${langName} where natural; otherwise keep original spelling.
...
async function translateChunks(chunks, langName) {
  const out = [];
  for (let i = 0; i < chunks.length; i++) {
    out.push(clean(await chat(sys(langName), chunks[i].join("\n"))));
  }
  return out;
}
...
async function translateFile(src, lang) {
  ...
  const dest = src.replace(/\.en\.ts$/, `.${lang}.ts`);
  const cacheDir = `/mnt/documents/trans-cache-${lang}`;
  ...
  let full = out.join("\n");
  // Fix dropped commas at chunk boundaries: a line that is just "}" followed by "{"
  full = full.replace(/^(\s*)\}\n(\s*\{)$/gm, "$1},\n$2");
  const v = await valid(full);
  fs.writeFileSync(dest, full);
  console.error(v === true ? `WROTE ${dest}` : `WROTE (invalid: ${String(v).slice(0,160)}) ${dest}`);
}
